...
a comment on the PR with a summary of which checks failed and why. The
comment groups failures by check name and shows the affected file and
error message in a table. A hidden HTML marker
(`<!-- pipelinerun-validation-comment -->`) is used to identify the
comment so subsequent runs update it in place rather than creating
duplicates. When validation passes after a previous failure, the comment
is automatically deleted.

The comment body is generated by pytest via `--validation-comment-file`
and posted using the `gh` CLI. The comment includes a link to the full
CI run logs.
