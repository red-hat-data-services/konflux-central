...
def generate_table(components: Dict[str, Set[str]], config: dict, output_format: str = 'markdown', metadata: Optional[dict] = None) -> str:
    ...
    if output_format in ('json', 'yaml'):
        ...
        if output_format == 'json':
            ...
            comment = (
                f"# Auto-generated by script/multi-arch-tracking/generate-table.py\n"
                f"# Do not edit manually — changes will be overwritten on the next run.\n"
                f"# Generated: {generated_at}\n"
            )
            return comment + yaml.dump(result, default_flow_style=False, sort_keys=False)
